...
.footnote[[UNEP-WCMC](https://www.fao.org/dryland-assessment/en/)]

```{r}
out <- vector("character", length = 6)
for (p in seq(1, 6)) {
  out[p] <- paste0("\n\n---\n# Catastrophic shifts\n```{r, message=FALSE, warning=FALSE, results='asis'}\nknitr::include_graphics('fig_ymca/catastrophic_shift", p ,".png')\n```\n.footnote[`r Citep(bib, 'kefi_local_2007')]")
}
```
...
for (p in seq(1, 4)) {
  out[p] <- paste0("\n\n---\nclass: center\n# Indirect facilitation\n```{r indir-fac",p,", message=FALSE, warning=FALSE, results='asis'}\nknitr::include_graphics('fig_ymca/indirect_facilitation", p ,".png')\n```\n")
}
```
...
---
class: center
# Transposing a dryland ecosystem to a cellular automata
...
.pull-left[
```{r, out.width="90%"}
knitr::include_graphics("fig_ymca/stability_u0.png")
```
]
...
.pull-right[
```{r, out.width="90%"}
knitr::include_graphics("fig_ymca/stability_u5.png")
```
]
...
```{r, out.width="100%"}
knitr::include_graphics("fig_ymca/bifurc_g=0.1_u=5.png")
```
]
...
modifies the transition of the protegee along the aridity gradient: from
catastrophic to continuous & hysteresis (less predictable)

--

does not necessarily affect species cooccurrence 

.footnote[`r Citep(bib, "danet_indirect_2020")`]
---
